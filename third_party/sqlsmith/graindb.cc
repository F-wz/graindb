...
graindb_connection::graindb_connection(string &conninfo) {
	// in-memory database
	database = make_unique<GrainDB>(nullptr);
	connection = make_unique<Connection>(*database);
}

void graindb_connection::q(const char *query) {
...
schema_graindb::schema_graindb(std::string &conninfo, bool no_catalog) : graindb_connection(conninfo) {
...
dut_graindb::dut_graindb(std::string &conninfo) : graindb_connection(conninfo) {
...
void dut_graindb::test(const std::string &stmt) {
	is_active = true;
