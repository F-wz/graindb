...
# make this data frame available as a view in graindb
...
rel = graindb.from_csv_auto(temp_file_name)
print(rel)
...
print(graindb.filter(test_df, 'i > 1'))
print(graindb.project(test_df, 'i +1'))
print(graindb.order(test_df, 'j'))
print(graindb.limit(test_df, 2))

print(graindb.aggregate(test_df, "sum(i)"))
print(graindb.distinct(test_df))

# when chaining only the first call needs to include the data frame parameter
print(graindb.filter(test_df, 'i > 1').project('i + 1').order('j').limit(2))
...
# create a table in graindb from the relation
...
res = graindb.query(test_df, 'my_name_for_test_df', 'SELECT * FROM my_name_for_test_df')
print(res.df())
