#include "nl_cwi_da_graindb_GrainDBNative.h"
#include "graindb.hpp"
...
JNIEXPORT jobject JNICALL Java_nl_cwi_da_graindb_GrainDBNative_graindb_1jdbc_1startup(JNIEnv *env, jclass,
                                                                                   jbyteArray database_j,
                                                                                   jboolean read_only) {
	auto database = byte_array_to_string(env, database_j);
	auto db = new GrainDB(database);
	return env->NewDirectByteBuffer(db, 0);
}

JNIEXPORT void JNICALL Java_nl_cwi_da_graindb_GrainDBNative_graindb_1jdbc_1shutdown(JNIEnv *env, jclass,
                                                                                 jobject db_ref_buf) {
	auto db_ref = (GrainDB *)env->GetDirectBufferAddress(db_ref_buf);
	if (db_ref) {
		delete db_ref;
	}
}

JNIEXPORT jobject JNICALL Java_nl_cwi_da_graindb_GrainDBNative_graindb_1jdbc_1connect(JNIEnv *env, jclass,
                                                                                   jobject db_ref_buf) {
	auto db_ref = (GrainDB *)env->GetDirectBufferAddress(db_ref_buf);
...
JNIEXPORT void JNICALL Java_nl_cwi_da_graindb_GrainDBNative_graindb_1jdbc_1set_1auto_1commit(JNIEnv *env, jclass,
...
JNIEXPORT jboolean JNICALL Java_nl_cwi_da_graindb_GrainDBNative_graindb_1jdbc_1get_1auto_1commit(JNIEnv *env, jclass,
                                                                                              jobject conn_ref_buf) {
	auto conn_ref = (Connection *)env->GetDirectBufferAddress(conn_ref_buf);
	if (!conn_ref) {
		jclass Exception = env->FindClass("java/sql/SQLException");
...
JNIEXPORT void JNICALL Java_nl_cwi_da_graindb_GrainDBNative_graindb_1jdbc_1disconnect(JNIEnv *env, jclass,
                                                                                   jobject conn_ref_buf) {
	auto conn_ref = (Connection *)env->GetDirectBufferAddress(conn_ref_buf);
	if (conn_ref) {
		delete conn_ref;
...
JNIEXPORT jobject JNICALL Java_nl_cwi_da_graindb_GrainDBNative_graindb_1jdbc_1prepare(JNIEnv *env, jclass,
                                                                                   jobject conn_ref_buf,
                                                                                   jbyteArray query_j) {
	auto conn_ref = (Connection *)env->GetDirectBufferAddress(conn_ref_buf);
	if (!conn_ref) {
...
JNIEXPORT jobject JNICALL Java_nl_cwi_da_graindb_GrainDBNative_graindb_1jdbc_1execute(JNIEnv *env, jclass,
	...
	if (param_len > 0) {
		...
		for (idx_t i = 0; i < param_len; i++) {
			...
			if (param == nullptr) {
				graindb_params.push_back(Value());
				continue;
			} else if (env->IsInstanceOf(param, bool_class)) {
				graindb_params.push_back(
				...
				graindb_params.push_back(Value(param_string));
	...
	res_ref->res = stmt_ref->stmt->Execute(graindb_params);
...
JNIEXPORT void JNICALL Java_nl_cwi_da_graindb_GrainDBNative_graindb_1jdbc_1release(JNIEnv *env, jclass,
                                                                                jobject stmt_ref_buf) {
	auto stmt_ref = (StatementHolder *)env->GetDirectBufferAddress(stmt_ref_buf);
	if (stmt_ref) {
		delete stmt_ref;
...
JNIEXPORT void JNICALL Java_nl_cwi_da_graindb_GrainDBNative_graindb_1jdbc_1free_1result(JNIEnv *env, jclass,
                                                                                     jobject res_ref_buf) {
...
JNIEXPORT jobject JNICALL Java_nl_cwi_da_graindb_GrainDBNative_graindb_1jdbc_1meta(JNIEnv *env, jclass,
                                                                                jobject stmt_ref_buf) {
	...
	jclass meta = env->FindClass("nl/cwi/da/graindb/GrainDBResultSetMetaData");
...
JNIEXPORT jobjectArray JNICALL Java_nl_cwi_da_graindb_GrainDBNative_graindb_1jdbc_1fetch(JNIEnv *env, jclass,
                                                                                      jobject res_ref_buf) {
	auto res_ref = (ResultHolder *)env->GetDirectBufferAddress(res_ref_buf);
	...
	auto vec_array = (jobjectArray)env->NewObjectArray(res_ref->chunk->column_count(),
	                                                   env->FindClass("nl/cwi/da/graindb/GrainDBVector"), nullptr);
	for (idx_t col_idx = 0; col_idx < res_ref->chunk->column_count(); col_idx++) {
		...
		jclass vec_class = env->FindClass("nl/cwi/da/graindb/GrainDBVector");
...
JNIEXPORT jint JNICALL Java_nl_cwi_da_graindb_GrainDBNative_graindb_1jdbc_1fetch_1size(JNIEnv *, jclass) {
	return STANDARD_VECTOR_SIZE;
}

JNIEXPORT jstring JNICALL Java_nl_cwi_da_graindb_GrainDBNative_graindb_1jdbc_1prepare_1type(JNIEnv *env, jclass,
                                                                                         jobject stmt_ref_buf) {
