...
public class GrainDBResultSet implements ResultSet {

	private GrainDBPreparedStatement stmt;
	private GrainDBResultSetMetaData meta;

	private ByteBuffer result_ref;
	private GrainDBVector[] current_chunk;
	private int chunk_idx = 0;
	private boolean finished = false;
	private boolean was_null;

	public GrainDBResultSet(GrainDBPreparedStatement stmt, GrainDBResultSetMetaData meta, ByteBuffer result_ref) {
		this.stmt = stmt;
		this.result_ref = result_ref;
		this.meta = meta;
		current_chunk = GrainDBNative.graindb_jdbc_fetch(result_ref);
	...
	public synchronized void close() throws SQLException {
		if (result_ref != null) {
			GrainDBNative.graindb_jdbc_free_result(result_ref);
			result_ref = null;
		}
		stmt = null;
		meta = null;
